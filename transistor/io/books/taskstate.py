...
class _TaskState:
    ...
    - electricfuture.com        x 100
     ...
     We want to track the state of each of the 100 part number searches on each website.
     So we can answer the question, 'do we have results for part number xyz1
     on website 'mousekey.com' and 'digifuture.com' yet? How do they compare?

     Darn it, the search on 'digifuture.com' failed. We need to retry it or else
     investigate why the search failed.

    trackers = ['mousekey.com.cn', 'mousekey.com', 'digidog.com.cn', 'digidog.com',
    'electricfuture.cn', 'electricfuture.com']

    We want to keep track of the task status state of each part number search, for each
...
class TaskStateABC(ABC):
    """
    A mixin to DRY up this setup.
    """
...
class TaskTracker(TaskStateABC):
    """
